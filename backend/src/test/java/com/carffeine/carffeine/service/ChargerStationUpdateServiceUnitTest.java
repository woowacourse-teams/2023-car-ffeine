...
class ChargerStationUpdateServiceUnitTest {
    ...
    void 기존에_없던_충전소가_생기면_저장한다() {
        // given
        given(chargeStationRepository.findAll()).willReturn(new ArrayList<>());
    ...
    void 기존에_있던_충전소의_데이터가_변경되면_수정한다() {
        // given
        given(chargeStationRepository.findAll()).willReturn(new ArrayList<>(List.of(ChargeStationFixture.선릉역_충전소_충전기_2개_사용가능_1개)));
    ...
    void 기존에_없던_충전기가_생기면_저장한다() {
        // given
        given(chargeStationRepository.findAll()).willReturn(new ArrayList<>(List.of(ChargeStationFixture.선릉역_충전소_충전기_0개_사용가능_0개)));
        List<ChargeStation> updateStations = List.of(ChargeStationFixture.선릉역_충전소_충전기_2개_사용가능_1개);
